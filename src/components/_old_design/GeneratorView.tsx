      ...
      <div className="w-80 flex flex-col gap-4 shrink-0 overflow-y-auto pr-1">
        ...
        <div className="glass-panel p-4 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3">
            ...
            {mode === "asset" && (
              <label className="checkbox-label">
                <input type="checkbox" checked={referenceFile !== null && denoise < 0.4} onChange={(e) => {
                  if (e.target.checked) {
                    if (!referenceFile) alert("Please upload a Reference Image first to use ControlNet.");
                    else setDenoise(0.3);
                  } else {
                    setDenoise(0.7);
                  }
                ...
                <span className="flex items-center gap-1.5">
                  Consistent Character (ControlNet)
                  <Tooltip text="Locks the character pose and structure using the uploaded reference image. Costs 3 Credits.">
                    <Info className="w-3 h-3 text-[#475569] cursor-help" />
                  </Tooltip>
                </span>
              </label>
